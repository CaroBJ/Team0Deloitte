...
def car_data_load(Path_to_data, Path_to_landscapes, num_train, num_test):
    ...
    # Load Landscapes
    land_dir_list = [f for f in os.listdir(Path_to_landscapes) if not f.startswith('.')]
    land_dir_list.sort()

    total_real=168

    if num_train > total_real:
        num_train=total_real

    test_num=num_test

    dir_list = [f for f in os.listdir(Path_to_data) if not f.startswith('.')]
    dir_list.sort()
